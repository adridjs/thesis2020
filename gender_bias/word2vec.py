...
class Word2VecSettings:
    ...
    def __init__(self, **kwargs):
        ...
        self.dataset = kwargs.get('corpus', 'EuroParl')
...
class Word2VecTrainer:
    ...
    def train(self):
        sentences = open(f'{self.dd.corpus_folder}/'
                         f'{self.settings.dataset}.corpus.tc.{self.settings.language}')\
            .readlines()
        ...
        # save vectors
        model.wv.\
            save_word2vec_format(f'{self.settings.dataset}-{self.settings.model_name}'
                                 f'.txt', binary=False)
